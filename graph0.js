...
function reshix(container, options) {
    ...
    this.drawPolar = function(e, t, n, r) {
        ...
        var c = "";
        var h = false;
        for (this.vars.theta = t; this.vars.theta <= n; this.vars.theta += r) {
            a = e(this.vars);
            o = a * Math.cos(this.vars.theta);
            u = a * Math.sin(this.vars.theta);
    ...
    this.drawParametric = function(e, t, n, r, i) {
        var s, o, u, a, f, l;
        var c = false;
        this.context.beginPath();
        for (this.vars.s = n; this.vars.s <= r; this.vars.s += i) {
            u = e(this.vars);
            a = t(this.vars);
    ...
    this.parseEquation = function(eq, vars) {
        ...
        for (i = 0; i < tokens.length; i++) {
            ...
            } else if (tokens[i].length > 0 && tokens[i].match(/^[a-z][a-z0-9]*$/i) && RESHIX_MATH[tokens[i]] == undefined) {
    ...
    this.reDraw = function() {
        ...
        for (var e in this.plots) {
            this.context.strokeStyle = "#000000";
            switch (this.plots[e].type) {
                case RESHIX_TYPE_FUNCTION:
                    this.context.strokeStyle = this.plots[e].options.color;
                    this.drawFunction(this.plots[e].jeq);
                    break;
                case RESHIX_TYPE_POLAR:
                    this.context.strokeStyle = this.plots[e].options.color;
                    this.drawPolar(this.plots[e].jeq, this.plots[e].options.thetamin, this.plots[e].options.thetamax, this.plots[e].options.thetastep);
                    break;
                case RESHIX_TYPE_PARAMETRIC:
                    this.context.strokeStyle = this.plots[e].options.color;
                    this.drawParametric(this.plots[e].jeqx, this.plots[e].jeqy, this.plots[e].options.smin, this.plots[e].options.smax, this.plots[e].options.sstep);
                    break;
                case RESHIX_TYPE_POINTS:
                    this.context.fillStyle = this.plots[e].options.color;
                    this.drawPoints(this.plots[e].eq);
                    break
            }
        }
        if (RESHIX_MSIE) {
    ...
    this.addPlot = function(e, t, n) {
        if (!t) t = RESHIX_TYPE_FUNCTION;
        if (!n) n = {};
        ...
        switch (t) {
            ...
            case RESHIX_TYPE_POLAR:
                ...
                n["thetamin"] = n["thetamin"] ? this.parseConst(n["thetamin"]) : 0;
                n["thetamax"] = n["thetamax"] ? this.parseConst(n["thetamax"]) : 2 * this.vars.pi;
                n["thetastep"] = n["thetastep"] ? this.parseConst(n["thetastep"]) : .01;
                if (n["thetastep"] <= 0) n["thetastep"] = .01;
                this.plots.push({
                    id: this.plotlastid++,
                    type: t,
                    eq: e,
            ...
            case RESHIX_TYPE_PARAMETRIC:
                ...
                n["smin"] = n["smin"] ? this.parseConst(n["smin"]) : 0;
                n["smax"] = n["smax"] ? this.parseConst(n["smax"]) : 10;
                n["sstep"] = n["sstep"] ? this.parseConst(n["sstep"]) : .01;
                this.plots.push({
                    id: this.plotlastid++,
                    type: t,
                    eq: e,
...
RESHIX_MATH = {};
RESHIX_MATH.sin = Math.sin;
RESHIX_MATH.cos = Math.cos;
RESHIX_MATH.tan = Math.tan;
RESHIX_MATH.asin = Math.asin;
RESHIX_MATH.acos = Math.acos;
RESHIX_MATH.atan = Math.atan;
RESHIX_MATH.abs = Math.abs;
RESHIX_MATH.floor = Math.floor;
RESHIX_MATH.ceil = Math.ceil;
RESHIX_MATH.exp = Math.exp;
RESHIX_MATH.sqrt = Math.sqrt;
RESHIX_MATH.max = Math.max;
RESHIX_MATH.min = Math.min;
...
RESHIX_MATH.ln = function(e) {
    return Math.log(e)
};
RESHIX_MATH.log = function(e) {
    return Math.log(e) / Math.log(10)
};
RESHIX_MATH.sinh = function(e) {
    return (Math.exp(e) - Math.exp(-e)) / 2
};
RESHIX_MATH.cosh = function(e) {
    return (Math.exp(e) + Math.exp(-e)) / 2
};
RESHIX_MATH.tanh = function(e) {
    return (Math.exp(e) - Math.exp(-e)) / (Math.exp(e) + Math.exp(-e))
};
RESHIX_MATH.asinh = function(e) {
    return Math.log(e + Math.sqrt(e * e + 1))
};
RESHIX_MATH.acosh = function(e) {
    return Math.log(e + Math.sqrt(e * e - 1))
};
RESHIX_MATH.atanh = function(e) {
...
function doPlot() {
    ...
    $("#plots").children("li").each(function() {
        tmp_fn_names += " " + $(this).children(".data-eq").val();
...
function writeState() {
    ...
    $("#plots").children("li").each(function() {
        ...
        switch (parseInt($(this).data("type"))) {
            case RESHIX_TYPE_FUNCTION:
                t["type"] = RESHIX_TYPE_FUNCTION;
                t["eq"] = $(this).children(".data-eq").val().replace(/ /g, "");
                t["color"] = rgb2hex($(this).find(".data-color").val());
                e.push(t);
                break;
            case RESHIX_TYPE_POLAR:
                t["type"] = RESHIX_TYPE_POLAR;
                t["eq"] = $(this).children(".data-eq").val().replace(/ /g, "");
                t["color"] = $(this).find(".data-color").val();
                t["thetamin"] = $(this).children(".data-thetamin").val();
                t["thetamax"] = $(this).children(".data-thetamax").val();
                t["thetastep"] = $(this).children(".data-thetastep").val();
                e.push(t);
                break;
            case RESHIX_TYPE_PARAMETRIC:
                t["type"] = RESHIX_TYPE_PARAMETRIC;
                t["eqx"] = $(this).children(".data-eqx").val().replace(/ /g, "");
                t["eqy"] = $(this).children(".data-eqy").val().replace(/ /g, "");
                t["color"] = $(this).find(".data-color").val();
                t["smin"] = $(this).children(".data-smin").val();
                t["smax"] = $(this).children(".data-smax").val();
                t["sstep"] = $(this).children(".data-sstep").val();
                e.push(t);
                break;
            case RESHIX_TYPE_POINTS:
                lines = $(this).find(".data-eq").val().split(/\n/);
    ...
    window.location.hash = "#" + Base64.encode(JSON.stringify(e));
    plotState = e
}
...
function readState() {
    ...
    for (i in plotState) {
        switch (parseInt(plotState[i]["type"])) {
            case RESHIX_TYPE_FUNCTION:
                e = $('<li class="plot ui-widget-content">' + $("#plot_template_" + RESHIX_TYPE_FUNCTION).html() + "</li>").data("type", RESHIX_TYPE_FUNCTION);
                e.children(".data-eq").val(plotState[i]["eq"]);
                if (plotState[i]["color"] == undefined) plotState[i]["color"] = "#000000";
                e.find(".data-color").val(plotState[i]["color"]).miniColors({
                    change: changeColorTimeout
                });
                e.find(".data-color").val(plotState[i]["color"]).miniColors("value", plotState[i]["color"]);
                $("#plots").append(e);
                break;
            case RESHIX_TYPE_POLAR:
                e = $('<li class="plot ui-widget-content">' + $("#plot_template_" + RESHIX_TYPE_POLAR).html() + "</li>").data("type", RESHIX_TYPE_POLAR);
                e.children(".data-eq").val(plotState[i]["eq"]);
                if (plotState[i]["color"] === undefined) plotState[i]["color"] = "#000000";
                e.find(".data-color").val(plotState[i]["color"]).miniColors({
                    change: changeColorTimeout
                ...
                e.children(".data-thetamin").val(plotState[i]["thetamin"]);
                if (plotState[i]["thetamax"] === undefined) plotState[i]["thetamax"] = "6";
                e.children(".data-thetamax").val(plotState[i]["thetamax"]);
                if (plotState[i]["thetastep"] === undefined) plotState[i]["thetastep"] = ".01";
                e.children(".data-thetastep").val(plotState[i]["thetastep"]);
                $("#plots").append(e);
                break;
            case RESHIX_TYPE_PARAMETRIC:
                e = $('<li class="plot ui-widget-content">' + $("#plot_template_" + RESHIX_TYPE_PARAMETRIC).html() + "</li>").data("type", RESHIX_TYPE_PARAMETRIC);
                e.children(".data-eqx").val(plotState[i]["eqx"]);
                e.children(".data-eqy").val(plotState[i]["eqy"]);
                if (plotState[i]["color"] === undefined) plotState[i]["color"] = "#000000";
                e.find(".data-color").val(plotState[i]["color"]).miniColors({
                    change: changeColorTimeout
                });
                ...
                e.children(".data-smin").val(plotState[i]["smin"]);
                if (plotState[i]["smax"] === undefined) plotState[i]["smax"] = "2pi";
                e.children(".data-smax").val(plotState[i]["smax"]);
                if (plotState[i]["sstep"] === undefined) plotState[i]["sstep"] = ".01";
                e.children(".data-sstep").val(plotState[i]["sstep"]);
                $("#plots").append(e);
                break;
            case RESHIX_TYPE_POINTS:
                e = $('<li class="plot ui-widget-content">' + $("#plot_template_" + RESHIX_TYPE_POINTS).html() + "</li>").data("type", RESHIX_TYPE_POINTS);
                pointstext = "";
                for (j in plotState[i]["eq"]) {
                    pointstext += plotState[i]["eq"][j].join() + "\n"
                }
                e.find(".data-eq").val(pointstext);
                if (plotState[i]["color"] === undefined) plotState[i]["color"] = "#000000";
                e.find(".data-color").val(plotState[i]["color"]).miniColors({
                    change: changeColorTimeout
                });
...
$(document).ready(function() {
    ...
    theplot = new reshix(document.getElementById("theplot"));
    theplot.reDraw();
